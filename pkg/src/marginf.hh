...
class MargInf;
class MargPhiCompute {
public:
  MargPhiCompute(const Env &env, gsl_rng **r, 
	     const uint32_t &iter,
...
inline void
MargPhiCompute::reset(uint32_t loc)
{
  _v.zero();
  _phidad.zero();
  _phimom.zero();
...
MargPhiCompute::update_lambda()
{
  ...
  for (uint32_t k = 0; k < _k; ++k) {
	...
	continue;
      lambdad[k][0] += phimomd[i][k] * snpd[i][_loc];
      lambdad[k][1] += phidadd[i][k] * (2 - snpd[i][_loc]);
    }
  }
  PopLib::set_dir_exp(_lambda, _Elogbeta);
}
...
inline void
MargPhiCompute::update_phimom(uint32_t n)
{
  //_phinext.zero();
...
inline void
MargPhiCompute::update_phidad(uint32_t n)
{
  //_phinext.zero();
  const double ** const elogthetad = _Elogtheta.const_data();
  const double ** const elogbetad = _Elogbeta.const_data();
...
inline void
MargPhiCompute::update_phis_until_conv()
{
  double u = 1./_k;
...
inline void
MargInf::estimate_theta(uint32_t n, Array &theta) const
{
...
inline double
MargInf::snp_likelihood(uint32_t loc, vector<uint32_t> &indivs, bool first)
{
...
inline void
MargInf::snp_likelihood(uint32_t loc, uint32_t n, Array &p)
{
  assert (p.n() == 3);
  assert (loc < _l);
  assert (n < _n);
...
inline bool
MargInf::kv_ok(uint32_t indiv, uint32_t loc) const
{
