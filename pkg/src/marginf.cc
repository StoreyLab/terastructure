...
MargInf::MargInf(Env &env, SNP &snp)
  :_env(env), _snp(snp),
...
MargInf::set_test_sample()
{
  ...
  Env::plog("(VAL1) total test snps (check)", _test_map.size());
}

void
MargInf::set_validation_sample2()
{
...
MargInf::infer()
{
  ...
  while (1) {
    ...
    _noderhot = pow(_nodetau0 + _nodec, -1 * _nodekappa);
    for (uint32_t i = 0; i < _n; ++i) {
      if (!kv_ok(i, _sampled_loc))
	continue;
      
      yval_t y = snpd[i][_sampled_loc];
      for (uint32_t k = 0; k < _k; ++k) {
	gd[i][k] = gd[i][k] + _noderhot *				\
	  (_alpha[k] + (scale * (y * phimomd[i][k] + (2 - y) * phidadd[i][k])) - gd[i][k]);
	assert (gd[i][k] >= .0);
      }
    }
    _nodec++;
...
double
MargInf::compute_likelihood(bool first, bool validation)
{
  uint32_t k = 0;
  double s = .0;
...
string
MargInf::add_iter_suffix(const char *c)
{
  ostringstream sa;
  if (_env.file_suffix)
    sa << c << "_" << _iter << ".txt";
...
void
MargInf::load_model(string betafile, string thetafile)
{
