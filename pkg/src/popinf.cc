...
PopInf::PopInf(Env &env, SNP &snp)
  :_env(env), _snp(snp),
   ...
   _pcomp(env, &_r, _iter, _n, _k, 0, _t,
	  _Elogtheta, _Elogf, _snp, _eta, *this),
   _shuffled_nodes(_n),
   _max_t(-2147483647),
   _max_h(-2147483647),
   _prev_h(-2147483647),
...
PopInf::batch_infer()
{
  ...
  while (1) {
    ...
    for (uint32_t l = 0; l < _l; ++l) {
      ...
      const Matrix &phi = _pcomp.phi();
      const double **phid = phi.data();
...
PopInf::infer()
{
  ...
  while (1) {
	...
	continue;
      ...
      for (uint32_t k = 0; k < _k; ++k) {
	gd[i][k] = gd[i][k] + _noderhot[i] * (_alpha[k] + (scale * phid[i][k]) - gd[i][k]);
	assert (gd[i][k] >= .0);
      }
      _nodec[i]++;
    }
...
PopInf::approx_log_likelihood()
{
  ...
  for (uint32_t l = 0; l < _l; ++l) {
	...
	continue;
      ...
      for (uint32_t k = 0; k < _k; ++k) {
	s += phid[n][k] * elogthetad[n][k] - phid[n][k] * log(phid[n][k]);
	s += phid[n][k] * (xd[n][l] * elogbetad[k][0] +			\
	     (2 - xd[n][l]) * elogbetad[k][1] + logcoeff(xd[n][l]));
...
double
PopInf::heldout_likelihood(bool first)
{
  uint32_t k = 0;
  double s = .0;
  SNPByLoc m;
...
double
PopInf::training_likelihood(bool first)
{
  uint32_t k = 0;
  double s = .0;
  SNPByLoc m;
...
void
PopInf::load_model(string betafile, string thetafile)
{
