...
PhiCompute::update_lambda()
{
  ...
  for (uint32_t k = 0; k < _k; ++k) {
	...
	phimomd[i][k] * _snp.mom(i,_loc);
      lambdad[k][1] += phidadd[i][k] * (_env.C - _snp.dad(i,_loc)) +	\
	phimomd[i][k] * (_env.C - _snp.mom(i,_loc));
    }
  }
  PopLib::set_dir_exp(_lambda, _Elogbeta);
  compute_Elogfdad();
...
PhiCompute::compute_Elogfdad()
{
  ...
  for (uint32_t n = 0; n < _n; ++n) {
	...
	double v = elogbetad[k][t] *					\
	  ((t == 0) ? _snp.dad(n,_loc) :  (_env.C - _snp.dad(n,_loc)));
	elogfd[n][k] += v;
      }
  }
}
...
PhiCompute::compute_Elogfmom()
{
  ...
  for (uint32_t n = 0; n < _n; ++n) {
    ...
    for (uint32_t k = 0; k < _k; ++k)
      for (uint32_t t = 0; t < _t; ++t) {
	double v = elogbetad[k][t] *					\
	  ((t == 0) ? _snp.mom(n,_loc) :  (_env.C - _snp.mom(n,_loc)));
	elogfd[n][k] += v;
      }
  }
}
...
FakePhase::snp_likelihood(uint32_t loc, uint32_t n, Array &p)
{
  ...
  for (uint32_t x = 0; x < 3; ++x) {
    ...
    double sum = .0;
    for (uint32_t k = 0; k < _k; ++k) {
      double v = gsl_sf_fact(_env.C) / 
	(gsl_sf_fact(x) * gsl_sf_fact(_env.C - x));
      //printf("v = %f\n", v);
      double m = v * pow(betad[loc][k], x) *			\
	pow(1 - betad[loc][k], _env.C - x) * thetad[n][k];
      sum += m;
    }
