#ifndef SNPSAMPLINGF_HH
#define SNPSAMPLINGF_HH
...
class SNPSamplingF;
class PhiRunnerF : public Thread {
public:
  PhiRunnerF(const Env &env, gsl_rng **r, 
	     const uint32_t &iter,
	     const uint32_t &x,
    ...
    : _env(env), _r(r), _iter(iter), _x(x),
      ...
      _idptr(NULL)
  { }
  ~PhiRunnerF() { if (_idptr) { delete _idptr; } } 
...
typedef std::map<pthread_t, PhiRunnerF *> ThreadMapF;

class SNPSamplingF {
public:
  SNPSamplingF(Env &env, SNP &snp);
  ~SNPSamplingF();
  ...
  bool init_phase() const { return _init_phase; }
...
private:
  ...
  LocIndivMap _loc_indiv_map;
  YArray _y;
  YArrayMap _heldout_loc_y;
};

inline void
PhiRunnerF::reset(uint32_t loc)
{
  _lambdat.zero();
  _loc = loc;
  _prev_iter = _iter;
...
inline void
PhiRunnerF::update_phimom(uint32_t n)
{
  //_phinext.zero();
...
inline void
PhiRunnerF::update_phidad(uint32_t n)
{
  //_phinext.zero();
  const double ** const elogthetad = _pop.Elogtheta().const_data();
  const double *** const elogbetad = _pop.Elogbeta().const_data();
...
inline double
SNPSamplingF::snp_likelihood(uint32_t loc, vector<uint32_t> &indivs, bool first)
{
  ...
  YArrayMap::const_iterator yitr = _heldout_loc_y.find(loc);
  assert(yitr != _heldout_loc_y.end());
  YArray *y =  yitr->second;
  assert(y);
...
inline void
SNPSamplingF::snp_likelihood(uint32_t loc, uint32_t n, Array &p)
{
  assert (p.n() == 3);
  assert (loc < _l);
  assert (n < _n);
...
inline bool
SNPSamplingF::kv_ok(uint32_t indiv, uint32_t loc) const
{
...
inline double
SNPSamplingF::logcoeff(yval_t x) {
  uint32_t c = 2;
