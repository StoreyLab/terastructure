...
template<> inline string
D1Array<unsigned char>::s() const
{
  ostringstream sa;
  uint32_t n = _n > ROW_LIMIT ? ROW_LIMIT : _n;
  sa << "\n[ ";
  for (uint32_t i = 0; i < n; ++i)
    sa << (unsigned char)(_data[i] + '0') << " ";
  if (_n > n)
    sa << "...";
  sa << "]";
  return sa.str();
...
template <class T> class D3Array;
...
public:
  ...
  int copy_from(uint32_t m, const D3Array<T> &a);
  ...
  string s() const;
  string s(uint32_t p) const { return ""; }
...
template<class T> inline int
D2Array<T>::copy_from(uint32_t m, const D3Array<T> &a)
{
  const T *** const d = a.data();
  for (uint32_t i = 0; i < a.n(); ++i)
...
template<> inline string
D2Array<double>::s(uint32_t p) const
{
  ostringstream sa;
  double *d = _data[p];
  uint32_t n = _n > ROW_LIMIT ? ROW_LIMIT : _n;
...
template<class T> inline void
sub(uint32_t m, const D3Array<T> &a, const D2Array<T> &b, D2Array<T> &c)
{
  const T *** const ad = a.data();
  const T ** const bd = b.data();
  T **cd = c.data();
...
template<class T> inline string 
D3Array<T>::s(uint32_t p) const
{
  assert (p < _m);
  double **data = _data[p];
  ostringstream sa;
  uint32_t n = (_n > ROW_LIMIT) ? ROW_LIMIT : _n;
  uint32_t k = (_k > COL_LIMIT) ? COL_LIMIT : _k;
  sa << "\n[ ";
  for (uint32_t i = 0; i < n; ++i) {
