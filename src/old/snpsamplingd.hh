#ifndef SNPSAMPLINGD_HH
#define SNPSAMPLINGD_HH
...
class SNPSamplingD;
class PhiRunner2 : public Thread {
public:
  PhiRunner2(const Env &env, gsl_rng **r, 
	     const uint32_t &iter,
	     const uint32_t &x,
...
typedef std::map<pthread_t, PhiRunner2 *> ThreadMap2;

class SNPSamplingD {
public:
  SNPSamplingD(Env &env, SNP &snp);
  ~SNPSamplingD();
...
inline void
PhiRunner2::update_phimom(uint32_t n)
{
  //_phinext.zero();
...
inline void
PhiRunner2::update_phidad(uint32_t n)
{
  //_phinext.zero();
  const double ** const elogthetad = _pop.Elogtheta().const_data();
  const double *** const elogbetad = _pop.Elogbeta().const_data();
...
inline double
SNPSamplingD::snp_likelihood(uint32_t loc, vector<uint32_t> &indivs, bool first)
{
...
inline void
SNPSamplingD::snp_likelihood(uint32_t loc, uint32_t n, Array &p)
{
  assert (p.n() == 3);
  assert (loc < _l);
  assert (n < _n);
...
inline bool
SNPSamplingD::kv_ok(uint32_t indiv, uint32_t loc) const
{
...
inline double
SNPSamplingD::logcoeff(yval_t x) {
  uint32_t c = 2;
